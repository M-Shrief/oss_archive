...
import tarfile
### 
from oss_archive.config import COMPRESSED_ARCHIVE_BASE_PATH
from oss_archive.utils.logger import logger

async def compress(oss_fullname: str) -> bool:
    """compress OSS directory"""
    try:
        if COMPRESSED_ARCHIVE_BASE_PATH is None:
            raise Exception("Compressed Archive's path is not defined")
            
        path = f"{COMPRESSED_ARCHIVE_BASE_PATH}/{oss_fullname}"
...
async def decompress(oss_fullname: str) -> bool:
    ...
    try:
        path = f"{COMPRESSED_ARCHIVE_BASE_PATH}/{oss_fullname}.tar"
        if COMPRESSED_ARCHIVE_BASE_PATH is None:
            raise Exception("Compressed Archive's path is not defined")

        with tarfile.open(path, "r") as tar:
            tar.extractall(path=COMPRESSED_ARCHIVE_BASE_PATH)
        return True
    except Exception as e:
